...
class BeamNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.room_mlp = nn.Sequential(nn.Linear(in_features=7, out_features=32), # Dimensions(L,W,H), Absorption, RT60, MaxOrder
                                      nn.ReLU(),
        ...
        # Input the Beam Pattern generated by the Geometry-Based FIR Filters
        # Input shape = (Batch, Channel, Frequency Bins, Beam Angles)

        self.filtered_beam_cnn = nn.Sequential(nn.Conv2d(in_channels=1,
                                                         out_channels=16,
                                                         kernel_size=(3,3),
                                                         padding=1),
                                               nn.ReLU(),
        ...
        # FIR features        = 64
        # Beam features       = 64
        # FIR Beam Features   = 64
        #
        # Total features      = 256

        self.fc = nn.Sequential(nn.Linear(in_features=256, out_features=256),
                                nn.ReLU(),
        ...
        self.output = nn.Linear(in_features=512,
                                out_features=100*24*64)
    # ====================================================
    # Define Propagation Through Network
    def forward(self, room, trajectory, fir, beam, filtered_beam):
        ...
        # FIR Beam Pattern Feature Extraction
        filtered_beam_features = self.filtered_beam_cnn(filtered_beam)
        filtered_beam_features = t.flatten(filtered_beam_features,
                                           start_dim=1)
        ...
        correction = correction.view(-1, 100, 24, 64)
